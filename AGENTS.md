...
- Live webcam exposes an InsightFace pack selector (`buffalo_l`, `buffalo_m`, `buffalo_s`) and swapper precision selector (`fp32`, `fp16`); keep `buffalo_l`/`fp32` as the safest baseline for `inswapper_128`, and treat `buffalo_m`/`buffalo_s` plus `fp16` as experimental speed options. Low-risk Live tuning controls can be hot-changed while Live runs; camera/source/capture/model controls are restart-only and should stay disabled during a run.
