...
## Action items
- [x] Address review blockers: make rejected hot-change payloads non-fatal, invalidate stopped prepare runs, clamp finite float hot-change values, and keep pipeline-frame changes from stalling behind backpressure.
...
- [x] Run minimal syntax validation only: `.\.venv_build\Scripts\python.exe -m py_compile .\windows_app\live_webcam.py .\windows_app\live_preview.py .\colab_api.py`.
...
## Validation
- [x] `.\.venv_build\Scripts\python.exe -m py_compile .\windows_app\live_webcam.py .\windows_app\live_preview.py .\colab_api.py`
