...
- Replace raw Live capture width/height controls with Capture mode plus Send scale: custom capture dimensions are requested from OBS/OpenCV at Live startup, DirectShow is the default Windows OBS backend, Auto capture mode requests the current OBS profile canvas size when available, and Send scale remains hot-change before WebSocket encoding.
