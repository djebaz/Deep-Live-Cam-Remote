...
## Changed
- Continue Windows app modularization by extracting settings, API client, Qt worker, and shared window core ownership from the PR #5 compatibility layer, then replacing broad `app_base as base` GUI imports with explicit module imports and removing Live webcam previous-function capture.
