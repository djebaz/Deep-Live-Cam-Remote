...
- Live webcam detects the actual decoded camera frame size, exposes a Capture scale selector (auto, 1x, 3/4x, 2/3x, 1/2x, 1/3x, 1/4x, custom exact width/height), and can send/receive JPEG or WebP frames with codec and quality values reported in backend diagnostics/perf logs. Low-risk Live tuning controls (capture scale/custom send size, codec/quality, process width, detection cadence, blending/mask options, many-faces, pipeline frames, and preview buffer/size) can be adjusted while Live is running; camera/source/FPS/model controls remain restart-only and are disabled during a run.
