...
- Live webcam can choose the OpenCV capture backend (DirectShow by default for OBS on Windows), then either use the negotiated camera mode or request a custom OBS/OpenCV capture size at startup before applying a hot-change Send scale (auto, 1x, 3/4x, 2/3x, 1/2x, 1/3x, 1/4x) before sending JPEG/WebP frames with codec and quality values reported in backend diagnostics/perf logs. Low-risk Live tuning controls (send scale, codec/quality, process width, detection cadence, blending/mask options, many-faces, pipeline frames, and preview buffer/size) can be adjusted while Live is running; camera/source/capture-backend/capture-size/FPS/model controls remain restart-only and are disabled during a run.
